...
def generate_chart():
    ...
    ax1.set_ylabel("No. of Boxes", color='#22258d', fontsize=13, labelpad=15)
    ax1.tick_params(axis='y', colors='#22258d', labelsize=11)
    ax1.set_ylim(0, max(boxes) * 1.4)
...
@app.route("/growerChart", methods=["POST"])
def generate_grower_chart():
    """Generate a modern styled bar-line chart and return as raw image bytes (UInt8List)"""
    # 1. Get and validate data
    try:
        data = request.get_json()
        ...
        quality = data.get('quality', 'Standard')
        weights = data.get('weights', [1200, 1550, 950, 1800, 1300, 1650])
        prices=data.get('prices',[90,90,65,45,45,110])
    except Exception as e:
        # Catch potential issues if data is not a dict or keys are missing
        return jsonify({"error": f"Invalid JSON data: {e}"}), 400
    ...
    if not (len(categories) == len(prices) == len(weights)):
        return jsonify({"error": "All lists ('categories', 'weights', 'prices') must be of the same length."}), 400
    ...
    bars = ax1.bar(categories, weights, 
                   color=bar_colors,
                   alpha=0.9, 
                   width=0.5,
                   edgecolor='white',
    ...
    ax1.set_ylabel("Weight in Kg", color='#22258d', fontsize=13, labelpad=15)
    ax1.tick_params(axis='y', colors='#22258d', labelsize=11)
    ax1.set_ylim(0, max(weights) * 1.4),

    ax2 = ax1.twinx()

    # 3. Line Plot (Weights) - Dark Blue
    line_color = '#22258d'
    ax2.plot(categories, prices, 
             color=line_color,
             marker='D',
             markersize=8,
             markeredgecolor='white',
    ...
    # Add data labels
    for x, y in zip(categories, prices):
        ax2.annotate(f"Rs. {y:,}", (x, y),
                     textcoords="offset points", xytext=(0,15), ha='center',
                     fontsize=10, fontweight='bold', color='white',
                     bbox=dict(boxstyle='round,pad=0.3', facecolor=line_color, edgecolor='white', alpha=0.9))

    ax2.set_ylabel("Price per Kg (Rs.)", color=line_color, fontsize=13, labelpad=15)
    ax2.tick_params(axis='y', colors=line_color, labelsize=11)
    ...
    fig.suptitle(f"PROCUREMENT ANALYTICS - {quality} Quality", 
                fontsize=18, y=0.98, color='#000000', fontweight='bold')
    plt.title("Weight vs Price by Consignment", 
             fontsize=12, pad=20, color='#000000', fontweight='bold')
    ax1.tick_params(axis='x', rotation=0, labelsize=11, colors='#555555')

    # 6. Data Table with colored columns
    cell_text = [[f"{w:,}" for w in weights], prices]
    table = plt.table(cellText=cell_text,
                      rowLabels=['WEIGHT (Kg)', 'Prices'],
                      rowColours=[line_color, '#808080'],
                      colLabels=categories,
                      colColours=bar_colors,
                      cellLoc='center',
...
@app.route("/ladaniChart", methods=["POST"])
def generate_ladani_chart():
    """Generate a modern styled bar-line chart and return as raw image bytes (UInt8List)"""
    # 1. Get and validate data
    try:
        data = request.get_json()
        ...
        weights = data.get('weights', [1200, 1550, 950, 1800, 1300, 1650])
        prices=data.get('prices',[90,90,65,45,45,110])
        landingcost=data.get('landing',10)
    except Exception as e:
        # Catch potential issues if data is not a dict or keys are missing
        return jsonify({"error": f"Invalid JSON data: {e}"}), 400
    ...
    line1_color ="#df0707"
    ax2.plot(categories, landingcosts, 
             color=line1_color,
             marker='D',
             markersize=8,
             markeredgecolor='white',
    ...
    for x, y in zip(categories, landingcosts):
        ax2.annotate(f"Rs. {y:,}", (x, y),
                     textcoords="offset points", xytext=(0,15), ha='center',
                     fontsize=10, fontweight='bold', color='white',
                     bbox=dict(boxstyle='round,pad=0.3', facecolor=line1_color, edgecolor='white', alpha=0.9))

    ax2.set_ylabel("Price per Kg (Rs.)", color=line_color, fontsize=13, labelpad=15)
    ax2.tick_params(axis='y', colors=line_color, labelsize=11)
    ax2.set_ylim(0, max(landingcosts) * 1.25)
